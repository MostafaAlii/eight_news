<div class="btn-group align-center">
    ...
    <div class="dropdown-menu dropmenu-menu-right">
        ...
        <form action="{{route('tag.destroy', 'test')}}" method="POST">
            {{ method_field('delete') }}
            @csrf
        ...
        <form action="{{route('tag.changeStatus')}}" method="POST">
            @csrf
            <div class="checkbox checbox-switch btn-sm switch-success text-center">
                <label>
                    <input type="hidden" name="id" id="id" value="{{$id}}">
                    <input type="checkbox" name="status" value="1" {{ $status=='active' ? "checked" : "" }}
...
@include('dashboard.tag.btn.edit')
